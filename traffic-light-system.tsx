  ...
  updateTrafficData(currentPhase: "ns" | "ew", phaseDuration: number) {
    ...
    directions.forEach((dir) => {
      ...
      if (isActuallyGreen && data.vehicleCount > 0) {
        ...
        console.log(`${dir}: ${vehiclesPassing} vehicles passed (GREEN)`)
...
export default function TrafficLightSystem() {
  ...
  const getNextTransition = useCallback((): string | null => {
    ...
    console.log(`Current phase: ${currentPhase}, North: ${northState}, East: ${eastState}`)
  ...
  const simulationStep = useCallback(() => {
    ...
    console.log(`Phase: ${currentPhase}, Light: ${currentLightState}, Timer: ${phaseTimerRef.current}`)
    ...
    if (shouldSwitch) {
      ...
      if (nextTransition) {
        console.log(`Executing transition: ${nextTransition}`)
